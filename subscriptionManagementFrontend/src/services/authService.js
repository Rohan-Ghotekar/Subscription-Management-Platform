...
export const updateProfilePhotoAPI = async (imageFile) => {
  ...
  const response = await api.put('/api/user/profilephoto', formData, {
    headers: {
...
// ── REGISTER PROVIDER ─────────────────────────────────────────
// POST /api/auth/registerprovider
// Request: { email, password, fullName, mobile, businessName, serviceType, description }
// Response: { accessToken, refreshToken, role: "PROVIDER", fullName, avatarUrl, ... }
export const registerProviderAPI = async (providerData) => {
  const response = await api.post('/api/auth/registerprovider', providerData)
  return response.data
}

// ── GET ALL USERS (Provider) ──────────────────────────────────
// GET /api/provider/allusers
// Returns: [ { userId, email, fullName, avatarUrl, role, createdAt, updatedAt, mobile } ]
export const getProviderUsersAPI = async () => {
  const response = await api.get('/api/provider/allusers')
  return response.data
}
