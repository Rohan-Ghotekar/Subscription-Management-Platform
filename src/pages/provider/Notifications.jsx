...
const formatTime = (isoString) => {
  ...
  const utcTime = new Date(isoString + "Z");

  const diff = Date.now() - utcTime.getTime();
...
function Notifications() {
  ...
  const pagedNotifs = notifs.slice(
  ...
  return (
    <div>
      <ProviderNavbar />
      <div className="user-shell provider-notifications-shell">
        <ProviderSidebar unreadCount={unreadCount} />
        <main className="user-content provider-notifications-page">
          {/* Page header */}
          <div
          ...
          {/* Notifications list */}
          {!loading && !error && (
            <div className="data-table-card provider-notifications-card">
              <div className="provider-notifications-items">
                {notifs.length === 0 ? (
              ...
              {notifs.length > NOTIFS_PER_PAGE && (
                <div
                  className="provider-notifications-pagination"
                  style={{
                    display: "flex",
                    alignItems: "center",
                    justifyContent: "space-between",
                ...
                >
                  <span style={{ fontSize: "12px", color: "var(--text-muted)" }}>
                    Showing {(safePage - 1) * NOTIFS_PER_PAGE + 1}-{Math.min(safePage * NOTIFS_PER_PAGE, notifs.length)} of {notifs.length}
                  </span>
